...
public class CargoCarTest extends TestCase {
  ...
  private void _test__checkWeight_transitions0_source_state_objectTests0_test(final CargoCar it) {
    ...
    assertEquals("cc.getTotalWeight() == 5000 failed after cc.setCargoWeight(4000)", 5000, _totalWeight);
  ...
  private void _transition_exprAction__checkWeight_transitions0_actions0(final CargoCar it) {
    try {
      ...
      fail("cc.setCargoWeight(4000) failed: " + error.getMessage());
    }
  ...
  private void _test__checkWeight_transitions0_effect_state_objectTests0_test(final CargoCar it) {
    ...
    int _totalWeight = this.cc.getTotalWeight();
    assertEquals("cc.getTotalWeight() == 7000 failed after cc.setCargoWeight(4000)", 7000, _totalWeight);
    
    int _cargoWeight = this.cc.getCargoWeight();
    assertEquals("cc.getCargoWeight() == 4000 failed after cc.setCargoWeight(4000)", 4000, _cargoWeight);
